...
bool testWithInitialPoint(std::shared_ptr<BM> bm, double eps, double x, double y, int aprIndex, bool isFirstInitPoint) {
    ...
    /*int aprIndex = 4;
    double xHybridApr[dim];
    snowgoose::VecUtils::vecCopy(dim, &(rosPath[dim * aprIndex]), xHybridApr);
    int mbAprCalls = testMB(bm, eps, xHybridApr, a, b, xRos, rosCallCounts[aprIndex], rosCalls, isFirstPoint);
    int hybridAprCalls = rosCallCounts[aprIndex] + mbAprCalls;
    bool isAprResultClose = snowgoose::VecUtils::vecDistAbs(dim, xHybridApr, xRos) < 1e-2;*/
    ...
    for (int i = firstHybrid; i < firstHybrid + hybridCount; i++) {
        ...
        //std::cout << sumCalls << "\t";
        results[i - firstHybrid + 2] = sumCalls; 
    }
...
void testBench(std::shared_ptr<BM> bm, double eps, double x, double y, int aprIndex, bool isFirstBenchmark) {
    const int dim = bm->getDim();
    ...
    for (int i = 1; i <= steps; i++) {
        for (int j = 1; j <= steps; j++) {
            ...
            //std::cout << snowgoose::VecUtils::vecPrint(dim, xInit) << " \n";
            isFirstLaunch = testWithInitialPoint(bm, eps, xInit[0], xInit[1], aprIndex, isFirstLaunch);
            isFirstLaunch = false;
        }
    }
...
int main(int argc, char** argv) {
    ...
    for (auto bm : tests) {
        testBench(bm, eps, x, y, aprIndex, isFirstBenchmark);
        isFirstBenchmark = false;
    }
